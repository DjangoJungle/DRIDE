...
while camera.IsGrabbing():
    ...
    if grabResult.GrabSucceeded():
        ...
        # 确保所有颜色都检测到至少一个位置，否则使用旧位置
        if not pos_red:
            pos_red = [{'x': old_points_2d[0]['x'], 'y': old_points_2d[0]['y'], 'color': 'red'}]
        else:
            pos_red = [{'x': pos_red[0][0] + pos_red[0][2] / 2, 'y': pos_red[0][1] + pos_red[0][3] / 2, 'color': 'red'}]

        if not pos_blue:
            pos_blue = [{'x': old_points_2d[1]['x'], 'y': old_points_2d[1]['y'], 'color': 'blue'}]
        else:
            pos_blue = [{'x': pos_blue[0][0] + pos_blue[0][2] / 2, 'y': pos_blue[0][1] + pos_blue[0][3] / 2, 'color': 'blue'}]

        if len(pos_white) < 2:
            pos_white = [
                {'x': old_points_2d[2]['x'], 'y': old_points_2d[2]['y'], 'color': 'white'},
                {'x': old_points_2d[3]['x'], 'y': old_points_2d[3]['y'], 'color': 'white'}
            ]
        else:
            pos_white = [
        ...
        points_2d = [pos_red[0], pos_blue[0], pos_white[0], pos_white[1]]
